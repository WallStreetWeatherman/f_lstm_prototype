...
# Set a static seed
random_seed = 36  # or whatever seed you want to use
...
# Define a new function for creating a model with different hyperparameters
def create_model(optimizer='adam', lstm_units=50, dropout_rate=0.2):
    model = Sequential()
    model.add(LSTM(units=lstm_units, return_sequences=True, input_shape=(hyperparameters["sequence_length"], X_train.shape[2]), 
...
def main():
    ...
    param_grid = dict(batch_size=batch_size, epochs=epochs, optimizer=optimizer, lstm_units=lstm_units, dropout_rate=dropout_rate)
